...
#[AsCommand(
    name: 'app:scrape:moto2',
    description: 'Scrapes the Moto2 calendar and prints session start times.',
)]
final class ScrapeMoto2ScheduleCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            $sessions = $this->scraper->scrape($year, 'MT2', 'Moto2');
        ...
        if ($sessions === []) {
            $io->warning(sprintf('No MT2 sessions found for %d.', $year));
        ...
        $io->title(sprintf('Moto2 %d session schedule', $year));
        $table->render();
