...
#[AsCommand(
    name: 'app:scrape:moto3',
    description: 'Scrapes the Moto3 calendar and prints session start times.',
)]
final class ScrapeMoto3ScheduleCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            $sessions = $this->scraper->scrape($year, 'MT3', 'Moto3');
        ...
        if ($sessions === []) {
            $io->warning(sprintf('No MT3 sessions found for %d.', $year));
        ...
        $io->title(sprintf('Moto3 %d session schedule', $year));
        $table->render();
